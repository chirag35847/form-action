...
export default function Signup() {
  function singninAction(prevState, formData) {
    ...
    if(!hasMinLength(password, 6)){
       errors.push("password should have atleast 6 characters");
    }
  ...
  const [formState, formAction, pending] = useActionState(singninAction, {
    errors: null
  })
  ...
  return (
    <form action={formAction}>
      ...
      {
        formState?.errors?.length>0 && <ul className='control-error'>
          {
            formState.errors.map((x)=><li key={x}><p>{x}</p></li>)
          }
        </ul>
      }
